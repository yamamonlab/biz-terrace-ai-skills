...
**箱の幅は文字数から決める:** 全角1字 = 文字の大きさ、半角1字 = その0.55倍。`幅 = max(名前の最長行 × 15, 補足の最長行 × 12) + 左右の余白 32`、8の倍数に切り上げ（補足の行も必ず数える）。文字が箱からはみ出したら **箱を広げるか改行する**。文字を縮めない。文字は箱の中央（`text-anchor="middle"`）。
...
- **ラベルを付ける線は、その区間を「ラベルの字幅＋32」以上の長さにする。** 箱の間隔32のままラベルを置くと、ラベルが箱の下に隠れる。足りなければ箱を離すか、ラベルを線の上（y−14）に出す
...
**点の x 座標は計算で決める:** 点が n 個なら `x = 88 + i × 704 ÷ (n − 1)`（i = 0, 1, …）を8の倍数に丸める。間隔を目で詰めたり、日付の差で広げたりしない（原文に日付があっても、この図は順序だけを表す）。点は7つまで。出来事の文字は1行 `(間隔 − 16) ÷ 13` 字まで（間隔176なら12字、117なら7字）で改行し、3行まで。収まらないなら点を減らして2枚に分けるか、HTML版の `TIMELINE` を使う。
...
1. 文字が箱・図の外にはみ出していない（字数 × 大きさ ≦ 箱の幅 − 32）。線のラベルが箱に掛かっていない。隣り合う文字どうしが、文字の大きさの半分以上離れている（時系列の日付・出来事も）
